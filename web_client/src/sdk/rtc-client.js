...
export default class RtcClient {
  constructor(props) {
    ...
    this.handleLocalStreamStats = props.handleLocalStreamStats;
    this.handleRemoteStreamStats = props.handleRemoteStreamStats;
    this.handleLocalAudioReport = props.handleLocalAudioReport;
    this.handleRemoteAudioReport = props.handleRemoteAudioReport;
    this.handleSubtitleMessage = props.handleSubtitleMessage;
  ...
  bindEngineEvents() {
    ...
    this.engine.on(VERTC.events.onLocalStreamStats, this.handleLocalStreamStats);
    this.engine.on(VERTC.events.onRemoteStreamStats, this.handleRemoteStreamStats);
    this.engine.on(VERTC.events.onLocalAudioPropertiesReport, this.handleLocalAudioReport);
    this.engine.on(VERTC.events.onRemoteAudioPropertiesReport, this.handleRemoteAudioReport);
    this.engine.enableAudioPropertiesReport({ interval: 100 });

    const parseBinaryMsg = (event) => {
      try {
        const raw = event.message || event.data || event;
        const text = (raw instanceof ArrayBuffer || raw instanceof Uint8Array)
          ? new TextDecoder('utf-8').decode(raw)
          : typeof raw === 'string' ? raw : JSON.stringify(raw);
        const jsonStart = text.indexOf('{');
        if (jsonStart === -1) return;
        const parsed = JSON.parse(text.slice(jsonStart));
        this.handleSubtitleMessage(parsed);
      } catch (e) {
        console.warn('Failed to parse binary message', e);
      }
    };

    this.engine.on(VERTC.events.onRoomBinaryMessageReceived, parseBinaryMsg);
    this.engine.on(VERTC.events.onUserBinaryMessageReceived, parseBinaryMsg);

    if (VERTC.events.onSubtitleMessageReceived) {
      this.engine.on(VERTC.events.onSubtitleMessageReceived, (event) => {
        console.log('[Subtitle event]', event);
  ...
  removeEventListener() {
    ...
    this.engine.off(VERTC.events.onLocalStreamStats, this.handleLocalStreamStats);
    this.engine.off(VERTC.events.onRemoteStreamStats, this.handleRemoteStreamStats);
    this.engine.off(VERTC.events.onLocalAudioPropertiesReport, this.handleLocalAudioReport);
    this.engine.off(VERTC.events.onRemoteAudioPropertiesReport, this.handleRemoteAudioReport);
